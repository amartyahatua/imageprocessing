...
public class ImageGrowing1 
{
	...
	private static long threshold=35;
		...
		{	
			...
			for(int th=-1;th<=1;th++)
			{
				for(int tw=-1;tw<=1;tw++)
				{
					...
					{
						if (Math.abs(storePixelGreen[ry][rx]-seedGreen)<0)
						{ 
		...
		while(storeConnectedBlue.size() > 0)
		{
			...
			for(int th=-1;th<=1;th++)
			{
				for(int tw=-1;tw<=1;tw++)
				{
					...
					{
						if (Math.abs(storePixelBlue[ry][rx]-seedBlue)<0)
						{ 
...
/*----------------------------------------IMAGE RECONSTRUCTION-----------------------------------------------*/
		...
		File file1 = new File("C:/Documents and Settings/Administrator/Desktop/test/outGrow1.bmp");
		...
		storePixelRedOut = imgGrng.getRGB(file1,1);
		storePixelGreenOut= imgGrng.getRGB(file1,2);
		storePixelBlueOut = imgGrng.getRGB(file1,3);
...

		{
			for(int w = 1;w<width-1;w++)
			{
				...
				if(storePixelRedOut[h][w]==255)
					storePixelRed[h][w]=0;
				
				if(storePixelGreenOut[h][w]==255)
					storePixelGreen[h][w]=0;
				
				if(storePixelBlueOut[h][w]==255)
		...
		for(int h = 0;h<height;h++)
		{
			for(int w = 0;w<width;w++)
			{
				result[h][w]= ((storePixelRed[h][w] <<16 & 0xff0000)+
						(storePixelRed[h][w]<<8 & 0x00ff00 ) +
						(storePixelRed[h][w] & 0x0000ff));
		...
		outputFile = new File("out_initial2.bmp");     
