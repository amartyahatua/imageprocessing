...
public class chk 
{
	...
	public float[][] getLogTransform(float pixelValues[][], int height,int width)
	{
		...
		//System.out.println(logTransform[0][0]);
		return logTransform;
	}
	...
	public void getImage(float resultLogRed[][],float resultLogGreen[][],float resultLogBlue[][],int height ,int width) throws IOException
	{
		...
		for(int h = 0;h<height;h++)
		{
			for(int w = 0;w<width;w++)
			{
				...
				String x= String.valueOf(picEqualizedRedTemp[h][w]);
				out.write(x);
				//out.newLine();
				x= String.valueOf(picEqualizedGreenTemp[h][w]);
				//out.newLine();
				x = String.valueOf(picEqualizedBlueTemp[h][w]);
				//out.newLine();
		...
		//System.out.println(picEqualizedGreenTemp[0][0]);
	...
	public static void main(String args[]) throws IOException
	{
		...
		chk logTrnsfrm = new chk();


		File file = new File("C:/Documents and Settings/Administrator/Desktop/ah/wom2.gif");
