...
public class LogTransform 
{
	...
	public float[][] getLogTransform(float pixelValues[][], int height,int width)
	{
		...
		for(int h = 0; h<height; h++)
		{
			for(int w = 0; w<width ; w++)
			{
				logTransformTemp[h][w] = (float)Math.floor((constant*( (Math.log10(pixelValues[h][w]+1)))));                        ///(Math.log10(1.02195252256666132))
	...
	public static void main(String args[]) throws IOException
	{
		...
		File file = new File("C:/Documents and Settings/Administrator/Desktop/test/log.png");
