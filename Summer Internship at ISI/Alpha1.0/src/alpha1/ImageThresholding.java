...
public class ImageThresholding 
{
	...
	public String getThreshold(String path,String inputImageName,String extension)
	{
		...
		int thresholdRed=thrshld.findThreshold(storePixelRed, height, width, initialValue);
		int thresholdGreen=thrshld.findThreshold(storePixelGreen, height, width, initialValue);
		int thresholdBlue=thrshld.findThreshold(storePixelBlue, height, width, initialValue);
		...
		for(int h = 0; h<height; h++)
		{
			for(int w = 0; w<width ; w++)
			{
				if(storePixelRed[h][w]>thresholdRed)
				{
		...
		for(int h = 0; h<height; h++)
		{
			for(int w = 0; w<width ; w++)
			{
				if(storePixelGreen[h][w]>thresholdGreen)
				{
		...
		for(int h = 0; h<height; h++)
		{
			for(int w = 0; w<width ; w++)
			{
				if(storePixelBlue[h][w]>thresholdBlue)
				{
