...
public class HistogramEqualization{
	...
	//CDF = Cumulative Distribute if Function
	...
	public float [] picEqualized(float [] grayScale, float [] equalization, int w, int h)
	{
		...
		for(int i = 0; i<w; i++)
		{
			for(int j = 0; j<h; j++)
			{
				newGS [counter] = equalization[(int)grayScale[counter]];
				System.out.println(newGS[counter]);//convert
				counter++;
	...
	public static void main (String args[]) throws IOException
	{
		...
		File file = new File("C:/Documents and Settings/Administrator/Desktop/ah/hls.jpg");
		...
		for(int i=0;i<size;i++)
			System.out.println(picEqualized[i]);
